...
/// Split prefill time between the projections and the gated delta-rule scan.
public struct PrefillBench: Sendable {
  public struct Options: Sendable {
    public var model: URL
    public var chunks: [Int] = [512, 2048]
    public var iterations: Int = 6
    /// Share of the wide projections to prefill on the Neural Engine, or nil for none.
    public var ane: ANEOffload?
  ...
  public static func run(
    ...
    if case .fraction(let value) = options.ane, !(value > 0 && value <= 1) {
      throw BenchFailure(
        reason: "the Neural Engine share takes a fraction in (0, 1]; got \(value)")
    }

    let packURL = options.model
    try ANEOffload.apply(options.ane, pack: packURL)
    ...
    log(field("vocab", "\(text.vocabSize)"))
  ...
  private static func measure(
    ...
    options: Options, log: (String) -> Void
  ) throws {
    let text = bonsai.config.textConfig
    ...
    rows.append(
      Row(
        ...
        layers: linearLayers,
        seconds: time(options) { eval(inProjQKV(hidden)) }, bucket: .recurrent))
    rows.append(
      Row(
        label: "GDN  in_proj_z   \(inProjZ.inputDim)→\(inProjZ.outputDim)",
        layers: linearLayers,
        seconds: time(options) { eval(inProjZ(hidden)) }, bucket: .offloadable))
    ...
    rows.append(
      Row(
        ...
        layers: linearLayers,
        seconds: time(options) { eval(outProj(gated)) }, bucket: .other))
    ...
    rows.append(
      Row(
        ...
        layers: linearLayers,
        seconds: time(options) { eval(silu(conv1d(convInput, convWeight, groups: convDim))) },
        bucket: .other))
    ...
    rows.append(
      Row(
        ...
        layers: 1,
        seconds: time(options) { eval(bonsai.text.lastLogits(hidden)) }, bucket: .head))

    let tokens = MLXArray(Array(repeating: Int32(1), count: chunk)).reshaped([1, chunk])
    let measured = time(options, 2) {
      let cache = bonsai.text.makeCache(kvConfig: .full)
      eval(bonsai.text.hidden(inputs: tokens, cache: cache))
    }

    let allPositions = time(options) { eval(bonsai.text.lmHead(hidden)) }

    report(
      chunk: chunk, rows: rows, measured: measured, allPositions: allPositions, log: log)
  }

  private static func report(
    chunk: Int, rows: [Row], measured: Double, allPositions: Double, log: (String) -> Void
  ) {
    let modelled = rows.reduce(0) { $0 + $1.total }

    log("")
    log(rule)
    log("chunk = \(chunk) tokens")
    log(rule)
    log("  " + pad("component", 38) + "  per call  layers      total   share")
    ...
    log(rule)
    log("  " + pad("sum of parts", 38) + String(format: "%23.1f ms", modelled * 1000))
    log(
    ...
    log("")
    log(verdict("delta-rule scan", bucketTotal(.scan)))
    log(verdict("offloadable (gate/up + GDN z)", bucketTotal(.offloadable)))
    log(
  ...
  private static var rule: String { String(repeating: "─", count: 46) }

  private static func field(_ label: String, _ value: String, width: Int = 10) -> String {
    pad(label, width) + " " + value
  }
  ...
  private static func time(
    _ options: Options, _ count: Int? = nil, _ body: () -> Void
  ) -> Double {
    let runs = count ?? options.iterations
    body()
    let start = Date()
